...
\begin{frame}{Gaussian Star Model}
    ...
    \begin{block}{Star Image Model}
        ...
        \[
        S(x_i, y_i | v) = A \exp\left(-\frac{(x_i-x_c)^2}{2 \sigma_x^2} + \frac{(y_i-y_c)^2}{2\sigma_y^2}\right) + N
        \]
        where v = $(A, x_c, y_c, \sigma_x, \sigma_y, N)$:
        \begin{itemize} 
...
\begin{frame}{Fast Gaussian Fitting (FGF): Formulation}
    \begin{columns}
        ...
        \[
            g_i = \ln I(x_i, y_i) \approx \ln S(x_i, y_i)
        \]
...
\begin{frame}{Conclusion}
    \begin{block}{Summary}
        The proposed \textbf{Weighted Fast Gaussian Fitting (FGF)} algorithm solves the bias problem of standard linearization by introducing a \textbf{two-step SNR-weighted approach}.
    \end{block}
...
\begin{frame}{Future Works}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Replace the simple Low Pass Filter with a \textbf{Kalman Filter}.
            \item Would allow predictive tracking of periodic errors (PE) rather than just reactive smoothing.
        \end{itemize}
        
        \item \textbf{Autotuning:}
        \begin{itemize}
            \item Implement algorithms (e.g., Ziegler-Nichols) to automatically optimize PID constants ($K_p, K_i, K_d$) for different mount loads.
        \end{itemize}
    \end{itemize}
\end{frame}
