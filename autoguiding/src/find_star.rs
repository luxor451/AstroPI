...
fn get_window(width: u32, height: u32, pixels: &[u8], window_size : u32, old_star_pos : StarPosition) -> Vec<Vec<u8>> {
...
fn S(x_i : u32, y_i : u32, v :(f64, f64, f64, f64, f64)) -> f64 {
    ...
    return A * exp( - ((x_i as f64 - x_c).powi(2) / (2.0 * sigma_x.powi(2))) - ((y_i as f64 - y_c).powi(2) / (2.0 * sigma_y.powi(2))) );
}

fn z_i(x_i: u32, y_i : u32, v :(f64, f64, f64, f64, f64), window_pixels : &[Vec<u8>], threshold: u8) -> f64 {
    let measured = (window_pixels[y_i as usize][x_i as usize] as f64 - threshold as f64).max(0.0);
    return measured - S(x_i, y_i, v);
}

#[allow(non_snake_case)]
fn obj_function(v :(f64, f64, f64, f64, f64), window_pixels : &[Vec<u8>], window_size : u32, threshold: u8) -> f64 {
    let mut sum = 0.0;
...
impl CostFunction for GaussianFitProblem {
    ...
    fn cost(&self, p: &Self::Param) -> Result<Self::Output, Error> {
        ...
        Ok(obj_function(v, &self.window_pixels, self.window_size, self.threshold))
    }
}
...
impl Guider {
    ...
    #[allow(non_snake_case)]
    pub fn find_star_CG(&self, width: u32, height: u32, pixels: &[u8], old_star_pos : StarPosition) -> Option<StarPosition> {
    ...
    #[allow(non_snake_case)]
    pub fn find_star_GF(&self, width: u32, height: u32, pixels: &[u8], old_star_pos : StarPosition) -> Option<StarPosition> {
    ...
    #[allow(non_snake_case)]
    fn fast_gaussian_fit(&self, window_pixels: &[Vec<u8>], window_size: u32, U: &[(usize, usize)]) -> Option<(f64, f64, f64, f64, f64)> {
        ...
        // // Validate that center is within window
        // if x_c < 0.0 || x_c >= window_size as f64 || y_c < 0.0 || y_c >= window_size as f64 {
        //     return None;
        // }
    ...
    fn S_i(x_i : f64, y_i : f64, A : f64, x_c : f64, y_c : f64, sigma_x : f64, sigma_y : f64) -> f64 {
        let res = A * exp( - ((x_i as f64 - x_c).powi(2) / (2.0 * sigma_x.powi(2))) - ((y_i as f64 - y_c).powi(2) / (2.0 * sigma_y.powi(2))) );
        res
    }

    #[allow(non_snake_case)]
    fn N_i(x_i : f64, y_i : f64,window_pixels : &[Vec<u8>], A : f64, x_c : f64, y_c : f64, sigma_x : f64, sigma_y : f64) -> f64 {
        return (window_pixels[y_i as usize][x_i as usize] as f64) - Self::S_i(x_i, y_i, A, x_c, y_c, sigma_x, sigma_y);
    }

    #[allow(non_snake_case)]
    fn SNR_i(x_i : f64, y_i : f64,window_pixels : &[Vec<u8>], A : f64, x_c : f64, y_c : f64, sigma_x : f64, sigma_y : f64) -> f64 {
    ...
    #[allow(non_snake_case)]
    pub fn find_star_FGF(&self, width: u32, height: u32, pixels: &[u8], old_star_pos : StarPosition, T : u32) -> Option<StarPosition> {    
        ...
        let U: Vec<(usize, usize)> = pixel_positions.iter()
        ...
        let (x_c, y_c, _sigma_x, _sigma_y, _A) = match self.fast_gaussian_fit(&window_pixels, window_size, &U) {
            Some(params) => params,
            None => return None,
        };
        ...
        for y in 0..window_size as usize {
            for x in 0..window_size as usize {
                let snr = Self::SNR_i(x as f64, y as f64, &window_pixels, _A, x_c, y_c, _sigma_x, _sigma_y);
                if snr > T as f64 {
                    V.push((x, y));
                }
            }
        ...
        let (x_c_final, y_c_final) = if V.len() >= 10 {
            ...
            match self.fast_gaussian_fit(&window_pixels, window_size, &V) {
                Some((x_c_refined, y_c_refined, _, _, _)) => {
                    ...
                    if x_c_refined >= 0.0 && x_c_refined < window_size as f64 
                        && y_c_refined >= 0.0 && y_c_refined < window_size as f64 {
        ...
        Some(StarPosition {
            x: old_star_pos.x - (window_size as f64 / 2.0) + x_c_final,
            y: old_star_pos.y - (window_size as f64 / 2.0) + y_c_final,
        })
    }
}